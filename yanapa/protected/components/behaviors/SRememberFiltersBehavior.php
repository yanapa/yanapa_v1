...
class SRememberFiltersBehavior extends CActiveRecordBehavior {
    /**
	...
	*/
	...
	private $_gridPageSize = null;				// DEVINF - 26/03/2013
	
	private $_gridContentLanguage = null;		// DEVINF - 26/03/2013
	...
	private function getRememberScenario() {
    ...
    private function saveSearchValues() {
        ...
        foreach ($attributes as $attribute) {
			...
			// ----------------------------------------------------------------------------------
			// DEVINF - The following code was modified to manage AJAX requests - 26/03/2013
			// ----------------------------------------------------------------------------------
			// DEVINF - Original Code
            ...
            // }
			// DEVINF - Modified code (26/03/2013)
            if (isset($this->owner->$attribute)) { 
				if((isset($_GET['ajax']) and $_GET['ajax']==$this->_rememberScenario))		// Checking Ajax value (examples: language-grid), in order to set a new state and save on its own scenario
					Yii::app()->user->setState($this->getStatePrefix() . $attribute, $this->owner->$attribute);
            }
			// ----------------------------------------------------------------------------------
			// DEVINF - End of modified code to manage Ajax requests
			// ----------------------------------------------------------------------------------
    ...
    private function doReadSave() {
		...
		// DEVINF - 26/03/2013 added for saving standard CGridView pagination references managed through URL Parameter
		// store grid sorting order
		$this->setGridSort(false);
		// store grid active page
		$this->setGridActivePage();
		// DEVINF - end saving pagination references
	...
	private function setGridSort($forced=false)		// DEVINF
	{
			$key = get_class($this->owner).'_sort';
			$gridSort = null;
			if(  (isset($_GET['ajax']) and $_GET['ajax']==$this->getRememberScenario())) {
				if(!empty($_GET[$key])) $gridSort = $_GET[$key];
			}
			if (!isset($gridSort) || empty($gridSort)) $gridSort = Yii::app()->user->getState($this->getStatePrefix() . 'sort');				
			Yii::app()->user->setState($this->getStatePrefix() . 'sort', $gridSort);
			if(!isset($_GET['ajax']) || ($_GET['ajax']==$this->getRememberScenario() && $forced == true)) {
				$_GET[$key] = $gridSort;
			}
	}

	private function setGridActivePage()		// DEVINF 26/03/2013
	{	
		$key = get_class($this->owner).'_page';
		$gridActivePage = null;
		if(isset($_GET['ajax']) and $_GET['ajax']==$this->getRememberScenario()) {
			if(!empty($_GET[$key])) {
				$gridActivePage = $_GET[$key];
			} elseif (!empty($_GET["ajax"]) and $_GET['ajax']==$this->getRememberScenario()) {
				// page 1 passes no page number, just an ajax flag
				$gridActivePage = 1;
			}
		}
		if (!isset($gridActivePage) || empty($gridActivePage)) $gridActivePage = Yii::app()->user->getState($this->getStatePrefix() . 'page');
		if (!isset($gridActivePage) || empty($gridActivePage)) $gridActivePage = null;
		Yii::app()->user->setState($this->getStatePrefix() . 'page', $gridActivePage);
		if(!isset($_GET['ajax'])) {
			$_GET[$key] = $gridActivePage;
		}
	}
	
	public function getGridPageSize()		// DEVINF 26/03/2013
   {
		if (!isset($this->_gridPageSize) || empty($this->_gridPageSize)) $this->setGridPageSize();
		return $this->_gridPageSize;
   }
   
   	private function setGridPageSize($value=null)	// DEVINF 26/03/2013
   {
		if (isset($value) && !empty($value)) $this->_gridPageSize = $value;
		
		if (!isset($this->_gridPageSize) || empty($this->_gridPageSize)) {
			if(isset($_GET['ajax']) and $_GET['ajax']==$this->getRememberScenario()) {
				$key = get_class($this->owner).'_pageSize'; 
				if(!empty($_GET[$key])){																	
					$this->_gridPageSize = $_GET[$key];
					$this->setGridSort(true);
				}		
			}
		}
        if (!isset($this->_gridPageSize) || empty($this->_gridPageSize)) $this->_gridPageSize = Yii::app()->user->getState($this->getStatePrefix() . 'pageSize');				
		if (!isset($this->_gridPageSize) || empty($this->_gridPageSize)) $this->_gridPageSize = Yii::app()->params['defaultPageSize'];
		if (!isset($this->_gridPageSize) || empty($this->_gridPageSize)) $this->_gridPageSize=10;	
		Yii::app()->user->setState($this->getStatePrefix() . 'pageSize', $this->_gridPageSize);
   }
	
	public function getGridContentLanguage()		// DEVINF 26/03/2013
   {
		if (!isset($this->_gridContentLanguage) || empty($this->_gridContentLanguage)) $this->setGridContentLanguage();
		return $this->_gridContentLanguage;
   }
   
   	private function setGridContentLanguage($value=null)	// DEVINF 26/03/2013
   {
		if (isset($value) && !empty($value)) $this->_gridContentLanguage = $value;
		if (!isset($this->_gridContentLanguage) || empty($this->_gridContentLanguage)) {
			if(isset($_GET['ajax']) and $_GET['ajax']==$this->getRememberScenario()) {
				$key = get_class($this->owner).'_contentLanguage';
				if(!empty($_GET[$key])){																	
					$this->_gridContentLanguage = $_GET[$key];
				}		
			}
		}
        if (!isset($this->_gridContentLanguage) || empty($this->_gridContentLanguage)) $this->_gridContentLanguage = Yii::app()->user->getState($this->getStatePrefix() . 'contentLanguage');				
		if (isset($this->_gridContentLanguage) && !empty($this->_gridContentLanguage) && $this->_gridContentLanguage == Yii::app()->language) $this->_gridContentLanguage = null;
		Yii::app()->user->setState($this->getStatePrefix() . 'contentLanguage', $this->_gridContentLanguage);
   }
